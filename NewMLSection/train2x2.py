...
A hybrid 3D CNN + Transformer Decoder model for 48-target prediction.
Architecture:
  1. Scaled 3D ResNet Stem (64 channels) + Squeeze-and-Excitation (SE) blocks.
  2. Spatial flattening + Deterministic 3D Sinusoidal Positional Encoding.
  3. TPC FiLM conditioning on the 3D feature maps.
  4. Transformer Decoder (Pre-LN): 48 Learned Queries alternate between Self-Attention
...
def _open_group_r(path: str) -> zarr.hierarchy.Group:
    store = zarr.DirectoryStore(path) if os.path.isdir(path) else zarr.open(store=path, mode="r").store
...
class MultiZarrDataset(Dataset):
    def __init__(self, zarr_paths: Sequence[str], input_scale="none", target_scale=1e-3, upcast_to="float32"):
        ...
        for g in self.groups:
            if "tpc_ids" in g: self.tpcds.append(g["tpc_ids"])
...
class PerceiverDecoder(nn.Module):
    def __init__(self, embed_dim, num_targets=48, num_layers=4, num_heads=8, dropout=0.1):
        super().__init__()
...
class HybridPerceiver3D(nn.Module):
    def __init__(self, c_in=1, base_channels=64, embed_dim=512, num_targets=48, 
                 tpc_embed_dim=32, num_tpcs=8, num_decoder_layers=4, dropout=0.1):
        super().__init__()
        ...
        self.register_buffer('pos_embedding', create_3d_sinusoidal_pos(4, 16, 8, embed_dim))
...
def train_epoch(model, loader, opt, scaler, device, base_lr, warmup_steps, total_steps, 
    ...
    return (float(loss_tensor[0].item()) / max(1, int(loss_tensor[1].item()))), step
...
def parse_args():
    ...
    ap.add_argument("--out-dir", type=str, default="./runs/newhope3_run")
...
def main():
    ...
    if is_main_process():
        ...
        print("Starting 3D Cross-Attention Perceiver Training")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_loss, global_step = train_epoch(
            ...
            True, sentinel_scaled, zero_below_scaled
        )

        vm = evaluate_distributed(model, val_loader, device, True, stdzr, 
                                  1.0/args.target_scale, sentinel_scaled, zero_below_scaled)

        if is_main_process():
            print(f"Epoch {epoch+1:03d} | Train Loss: {train_loss:.4f} | Val MSE: {vm['mse']:.4f} | Val R2: {vm['r2']:.4f}")
            
            # Save regular checkpoint to resume from if interrupted
            torch.save({
                "epoch": epoch,
